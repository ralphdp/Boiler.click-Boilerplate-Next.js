...
import { Search, Save, Trash2, Plus, Globe, Shield, Image as ImageIcon } from "lucide-react";
import { GlassCard } from "@/components/ui/GlassCard";
import { Input } from "@/components/ui/Input";
import { Select } from "@/components/ui/Select";
import { useToast } from "@/components/ui/Toast";
import { getSEOMatrix, updateSEORoute } from "@/core/actions/admin";
...
export function AdminSEO({ t }: { t: any }) {
    ...
    const [matrix, setMatrix] = useState<SEORouteData[]>([]);
            ...
            <div className="bg-black/40 border border-white/5 relative overflow-hidden">
                ...
                <div className="p-4 md:p-6 border-b border-white/5 bg-white/5">
                    <div className="flex flex-col gap-1 shrink-0">
                        ...
                        <span className="text-[8px] font-mono text-white/30 uppercase tracking-[0.3em]">Decentralized Metadata Stewardship Protocol</span>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col sm:flex-row flex-wrap items-stretch lg:items-center gap-3 w-full lg:w-auto">
                        <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-2">
                            ...
                            <button
                                ...
                                className="h-[54px] px-6 bg-[var(--accent)]/20 border border-[var(--accent)]/50 text-[10px] font-black uppercase tracking-widest text-white hover:bg-[var(--accent)]/30 transition-all flex items-center justify-center gap-2 shrink-0"
                            >
                ...
                <div className="w-full max-h-[600px] overflow-y-auto overflow-x-auto admin-scrollbar">
                    {loading ? (
                        <div className="p-12 text-center text-white/20 animate-pulse uppercase tracking-[0.3em] text-[10px] font-black">Synchronizing Substrate...</div>
                    ) : filteredMatrix.length === 0 && !editingRoute ? (
                        <div className="p-12 text-center text-white/10 uppercase tracking-[0.3em] text-[10px] font-black h-[200px] flex items-center justify-center">No Route Overrides Defined</div>
                    ) : (
                        <table className="w-full min-w-[800px] text-left border-collapse text-xs font-mono">
                            <thead>
                                <tr className="bg-white/5 text-white/50 sticky top-0 z-10 backdrop-blur-md">
                                    <th className="p-4 font-normal tracking-widest uppercase">Identity (Route)</th>
                                    <th className="p-4 font-normal tracking-widest uppercase">Metadata Overrides</th>
                                    <th className="p-4 font-normal tracking-widest uppercase">Indexing</th>
                                        ...
                                        <td colSpan={4} className="p-6">
                                            <div className="space-y-4">
                                                <div className="flex justify-between items-center mb-4">
                                                    <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--accent)]">Editing Matrix Node: {editingRoute}</h4>
                                                    <div className="flex gap-2">
                                                        <button
                                                            onClick={() => setEditingRoute(null)}
                                                            className="px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-white/50 hover:text-white"
                                                        >
                                                            Cancel
                                                        </button>
                                                        <button
                                                            onClick={() => handleSave(editingRoute, editData)}
                                                            className="px-6 py-2 bg-[var(--accent)] border border-[var(--accent)]/50 text-[10px] font-black uppercase tracking-widest text-black hover:brightness-110 transition-all"
                                                        >
                                                            Broadcast Changes
                                                        </button>
                                                    </div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div className="space-y-4">
                                                        <div>
                                                            <label className="text-[9px] font-black uppercase tracking-widest text-white/30 block mb-2">Matrix Title</label>
                                                            <Input
                                                        ...
                                                        <div>
                                                            <label className="text-[9px] font-black uppercase tracking-widest text-white/30 block mb-2">Metadata Description</label>
                                                            <Input
                                                    ...
                                                    <div className="space-y-4">
                                                        <div>
                                                            <label className="text-[9px] font-black uppercase tracking-widest text-white/30 block mb-2">Keywords (CSV)</label>
                                                            <Input
                                                        ...
                                                        <div className="grid grid-cols-2 gap-4">
                                                            <div>
                                                                <label className="text-[9px] font-black uppercase tracking-widest text-white/30 block mb-2">OG Image URL</label>
                                                                <Input
                                                            ...
                                                            <div>
                                                                <label className="text-[9px] font-black uppercase tracking-widest text-white/30 block mb-2">Indexing Guard</label>
                                                                <Select
                                    ...
                                    <tr key={item.route} className="hover:bg-white/5 transition-colors group">
                                        <td className="p-4">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="font-mono text-xs font-black tracking-widest uppercase">{item.route}</span>
                                            </div>
                                        </td>
                                        <td className="p-4">
                                        ...
                                        <td className="p-4">
                                            {item.noIndex ? (
                                                <span className="text-[8px] px-2 py-0.5 bg-red-500/10 border border-red-500/30 text-red-500 font-black uppercase tracking-widest flex items-center gap-1 w-fit">
                                                    <Shield size={8} /> No Index
                                                </span>
                                            ) : (
                                                <span className="text-[8px] px-2 py-0.5 bg-green-500/10 border border-green-500/30 text-green-500 font-black uppercase tracking-widest flex items-center gap-1 w-fit">
                                                    <Globe size={8} /> Public
                                                </span>
                                            )}
                                        </td>
                                        <td className="p-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-white/5 border border-white/10 hover:border-white/30 text-[9px] font-black uppercase tracking-widest transition-all"
                                                >
                                                    Modify
                                                </button>
                                                <button
                                                    onClick={() => handleDelete(item.route)}
                                                    className="p-2 border border-red-500/20 hover:bg-red-500/10 text-red-500 transition-all"
                                                >
                                                    <Trash2 size={14} />
                                                </button>
                                            </div>
