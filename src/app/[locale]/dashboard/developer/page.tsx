...
export default function DeveloperPage() {
    ...
    const handleUnregisterWebhook = async (id: string, name: string) => {
        setConfirmModal({
            open: true,
            title: "REMOVE EVENT SINK",
            description: `SYSTEM WARNING: Are you sure you want to detach webhook '${name}'? This will immediately halt the event stream to the target URL.`,
            variant: "danger",
            requireCipher: true,
            action: async () => {
                setIsGenerating(true);
                // Placeholder: Implement actual unregisterServerAction here.
                // try {
                //      const res = await unregisterWebhook(id);
                //      ...
                // }
                // For now, optimistic UI update or just reload.
                toast({ title: "Sink Detached", description: `Webhook connection for ${name} has been severed.`, type: "success" });
                await loadWebhooks();
                setIsGenerating(false);
            }
        });
