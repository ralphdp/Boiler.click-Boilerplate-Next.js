...
import { setGlobalBroadcast } from "@/core/actions/system";
...
export function AdminBroadcast({
    ...
    const handleBroadcastSubmit = async (verified: boolean = false) => {
        ...
        const res = await setGlobalBroadcast(broadcastMessage, broadcastUrgency);
        if (res.success) {
    ...
    return (
        ...
        >
            <GlassCard className="border border-white/5 bg-black/40 p-6 flex flex-col justify-between gap-6">
                ...
                <div className="flex flex-col gap-6 w-full justify-start">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-widest text-white/30">Urgency Level</label>
                        <div className="flex gap-2">
                            {(['INFO', 'WARN', 'CRIT'] as const).map((level) => (
                                <button
                                    key={level}
                                    onClick={() => setBroadcastUrgency(level)}
                                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest border transition-all ${broadcastUrgency === level
                    ...
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-widest text-white/30">Broadcast Message</label>
                        <Input
                            type="text"
                    ...
                    <button
                        ...
                        className={`w-full py-4 text-xs font-black uppercase tracking-widest transition-all border ${broadcastUrgency === 'CRIT'
                            ? 'bg-red-600 hover:bg-red-700 border-red-500 text-white'
                            : 'bg-[var(--accent)] hover:bg-[var(--accent)]/80 border-[var(--accent)] text-black'
                            }`}
                    >
