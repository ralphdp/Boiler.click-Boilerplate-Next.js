    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <SolidCard className="border border-white/5 bg-black/40 p-6 flex flex-col justify-between gap-4 relative overflow-hidden group">
                    <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity"><Users size={120} /></div>
                    <h3 className="text-xs font-semibold tracking-normal] text-white/50 relative z-10">{t.admin.overview.totalNodes}</h3>
                    <div className="text-4xl font-mono text-[var(--accent)] font-bold relative z-10">{nodes.length}</div>
                    <div className="text-[10px] text-white/30 tracking-normal relative z-10">{t.admin.overview.activeCitizens}</div>
                </SolidCard>
                <SolidCard className="border border-white/5 bg-black/40 p-6 flex flex-col justify-between gap-4 relative overflow-hidden group">
                    <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity"><Activity size={120} /></div>
                    <h3 className="text-xs font-semibold tracking-normal] text-white/50 relative z-10">{t.admin.overview.storeMode}</h3>
                    <div className="flex flex-col gap-2 relative z-10">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {haltingProtocol && <span className="px-2 py-0.5 text-[9px] bg-red-500/20 text-red-500 border border-red-500/30 tracking-normal font-bold">{t.admin.overview.maintenance}</span>}
                            {preLaunchMode && <span className="px-2 py-0.5 text-[9px] bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 tracking-normal font-bold">{t.admin.overview.earlyAccess}</span>}
                            {sandboxMode && <span className="px-2 py-0.5 text-[9px] bg-blue-500/20 text-blue-500 border border-blue-500/30 tracking-normal font-bold">{t.admin.overview.sandbox}</span>}
                        </div>
                    </div>
                    <div className="text-[10px] text-white/30 tracking-normal relative z-10">{commerceMode === 'saas' ? `${pricingTiers.length} ${t.admin.overview.activeTiers}` : commerceMode === 'store' ? `${totalStoreProducts} ${t.admin.overview.activeProducts}` : t.admin.overview.currentPosture}</div>
                </SolidCard>
                <SolidCard className="border border-white/5 bg-black/40 p-6 flex flex-col justify-between gap-4 relative overflow-hidden group">
                    <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity"><ShieldCheck size={120} /></div>
                    <h3 className="text-xs font-semibold tracking-normal] text-white/50 relative z-10">{t.admin.overview.criticalActions}</h3>
                    <div className="text-4xl font-mono text-red-500 font-bold relative z-10">{traces.filter(t => t.severity === "CRIT").length}</div>
                    <div className="text-[10px] text-white/30 tracking-normal relative z-10">{t.admin.overview.criticalTraces}</div>
                </SolidCard>
            </div>
            <div className="bg-black/40 border border-white/5">
                <div className="p-4 border-b border-white/5 bg-white/5 flex justify-between items-center">
                    <h3 className="text-xs font-semibold tracking-normal]">{t.admin.overview.recentAudit}</h3>
                    <Button
                        variant="solid"
                        onClick={() => setTab("audit")}
                        className="text-[10px] text-[var(--accent)] hover:text-white transition-colors tracking-normal font-bold border-none bg-transparent hover:bg-white/5"
