    ...
    return (
        <main className="relative min-h-screen p-6 text-white overflow-hidden flex flex-col items-center">
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                {metrics.map((m, i) => (
                    <SolidCard key={i} className="p-8 space-y-4 border-white/5 hover:border-[var(--accent)]/30 transition-all">
                        <div className="text-[var(--accent)] opacity-50">
                            {/* Simple fallback since Lucide icons are being passed */}
                            <m.icon size={20} />
                        </div>
                        <div className="space-y-1">
                            <div className="text-3xl font-semibold font-mono">{m.value}</div>
                            <div className="text-[9px] font-semibold tracking-normal text-white/30">{m.label} // {m.sub}</div>
                        </div>
                    </SolidCard>
                ))}
            </div>
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-3 gap-6 pb-32">
                <SolidCard className="lg:col-span-2 p-12 space-y-8 min-h-[400px]">
                    <div className="flex justify-between items-center">
                        <h3 className="text-lg font-semibold tracking-normal flex items-center gap-2">
                            <TrendingUp size={16} className="text-[var(--accent)]" />
                            Throughput Pulse
                        </h3>
                        <div className="text-[10px] font-mono text-white/20 tracking-normal">REAL-TIME SYNCED</div>
                    </div>
                    {/* Simulated Chart */}
                    ...
                    <div className="grid grid-cols-2 gap-12 pt-8 border-t border-white/5">
                        <div className="space-y-2">
                            <div className="text-[10px] font-semibold tracking-normal text-white/40">Peak Execution</div>
                        ...
                        <div className="space-y-2">
                            <div className="text-[10px] font-semibold tracking-normal text-white/40">Network Health</div>
                            <div className="flex items-center gap-2 text-emerald-500">
                                <CheckCircle2 size={16} />
                                <span className="text-lg font-mono">Optimal</span>
                            </div>
                        </div>
                    </div>
                </SolidCard>

                <SolidCard className="p-12 space-y-12">
                    <h3 className="text-lg font-semibold tracking-normal flex items-center gap-2">
                    ...
                    <div className="space-y-8">
                        {[
                            ...
                            <div key={a.label} className="space-y-3">
                                <div className="flex justify-between text-[10px] font-semibold tracking-normal">
                                    <span>{a.label}</span>
